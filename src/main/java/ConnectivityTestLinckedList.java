...
public class ConnectivityTestLinckedList {
    ...
    public static void main(String[] args) {
        ConnectivityTestLinckedList connectivityTestLinkedList = new ConnectivityTestLinckedList();
        MemoryStorage data = null;
        try {
        ...
        ArrayList<OsmWay> ss = connectivityTestLinkedList.dataToWays(data);
        connectivityTestLinkedList.connectivityMethod(ss);
    ...
    private void connectivityMethod(ArrayList<OsmWay> inputWays){
        ...
        ArrayList<LinkedList<Long>> generalListOfNodesOfSubgraf = new ArrayList<>(); //Массив 1 уровня для Set`ов точек, по индексу они соответствуют соответствующим массивам в
        ...
        for (OsmWay osmWay: inputWays){              // берем попорядку веи из массива
            ...
            if(generalListOfNodesOfSubgraf.isEmpty()) {    // если главный массив пуст, то добавляем туда первый сет
                LinkedList<Long> tmp1 = new LinkedList<>();    // новый set для точек обрабатываемого вея
                ...
                for (LinkedList<Long> set : generalListOfNodesOfSubgraf) { // для каждого сета точек, которые уже находятся
                ...
                if (tempListForMergeSubgraf.size() > 0) {
                   ...
                   // добавляем точки вея к первому набору точек, в котором совпадение обнаружилось
                    LinkedList<Long> tmpSet = generalListOfNodesOfSubgraf.get(indexFforMerge);
                    for (long LL : nodeIDs) {
                        tmpSet.add(LL);
                    }
                    ...
                    LinkedList<Long> tmpSetFMerge = new LinkedList<>();
...
//                    }
                ...
                // или, если нечего объединять, добавить новый субграф
                    ...
                    LinkedList<Long> tmp = new LinkedList<>();    // новый set для точек обрабатываемого вея
        ...
        System.out.println("-----------------------");
        int size1 = 0;
        for (LinkedList<Long> set: generalListOfNodesOfSubgraf){
