...
public class ConnectivityTestArrayList {
    ...
    public static void main(String[] args) {
        ConnectivityTestArrayList connectivityTestArrayList = new ConnectivityTestArrayList();
        MemoryStorage data = null;
        try {
        ...
        ArrayList<OsmWay> ss = connectivityTestArrayList.dataToWays(data);
        connectivityTestArrayList.connectivityMethod(ss);
    ...
    private void connectivityMethod(ArrayList<OsmWay> inputWays){
        ...
        ArrayList<ArrayList<Long>> generalListOfNodesOfSubgraf = new ArrayList<>(); //Массив 1 уровня для Set`ов точек, по индексу они соответствуют соответствующим массивам в
        ...
        for (OsmWay osmWay: inputWays){              // берем попорядку веи из массива
            ...
            if(generalListOfNodesOfSubgraf.isEmpty()) {    // если главный массив пуст, то добавляем туда первый сет
                ArrayList<Long> tmp1 = new ArrayList<>();    // новый set для точек обрабатываемого вея
                ...
                for (ArrayList<Long> set : generalListOfNodesOfSubgraf) { // для каждого сета точек, которые уже находятся
                ...
                if (tempListForMergeSubgraf.size() > 0) {
                   ...
                   // добавляем точки вея к первому набору точек, в котором совпадение обнаружилось
                    ArrayList<Long> tmpSet = generalListOfNodesOfSubgraf.get(indexFforMerge);
                    for (long LL : nodeIDs) {
                        tmpSet.add(LL);
                    }
                    ...
                    ArrayList<Long> tmpSetFMerge = new ArrayList<>();
...
//                    }
                ...
                // или, если нечего объединять, добавить новый субграф
                    ...
                    ArrayList<Long> tmp = new ArrayList<>();    // новый set для точек обрабатываемого вея
        ...
        System.out.println("-----------------------");
        int size1 = 0;
        for (ArrayList<Long> set: generalListOfNodesOfSubgraf){
